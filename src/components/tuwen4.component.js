...
var tuwen1HTML = `<div class="tuwen-wrapper4 dragDiv" data-type="TuWen4" style="left:{{x}}px; top:{{y}}px;">
                    <div class="img">
                        {{if url!="###"}}
...
var _tuwen1HTML = `<div class="tuwen-wrapper4 dragDiv" data-type="TuWen4" style="left:{{x}}px; top:{{y}}px;">
...
let configHTML = `
                ...
                <div class="editor_banner editor_common">
                    ...
                    <div class="padTop15">
                        <h4>标题</h4>
                        <input maxlength="14" data-key="title,_title" value="{{_title}}" type="text" class="url_input" placeholder="">
                    </div>
                    <div class="padTop15">
                        <h4>描述</h4>
                        <textarea maxlength="44" data-key="desc,_desc" >{{_desc}}</textarea>
                    </div>
                    <div class="padTop15">
                        <h4>跳转链接</h4>
                        <input data-key="url,_url" data-key-url8="regURl" value="{{_url}}" type="text" class="url_input" placeholder="">
                    </div>
                </div>
                </div>
    `
...
export default class TestComponent{
    ...
    bindEvent(){           
        let _this=this;
        $(doc).off("input.tuwen4").on("input.tuwen4", "input[data-key],textarea[data-key]", function (e) {
            let $ele = $(this).closest(_this.options.configBox);
            let data=$ele.data('data');
            let type = data&&data.type           
            
            if(!data || type!=='TuWen4'){return}
            ...
            if(keys[0] == 'desc') {
                obj = {
                    [keys[0]]: this.value.substring(0,44).replace(/\u0020/g,'&nbsp;')
                }
            }
        ...
        $(doc).on('click',this.options.addBtn,()=>{
            ...
            tools.generate({
                ...
                title:"测试图文标题",
                url:"###",
                desc:"容内容标题内容标题内容内容标题内容标题内容题内容标题内容标题内容内容标题内容标题内容标题内容",
                x:tools.getX,
                y:tools.getY,
                fileName:'',
                config:{//侧右配置项数据
        ...
        $(doc).on("change.tuwen4", "input[data-key-url8]", function (e) {
